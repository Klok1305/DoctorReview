...
- Ручная публикация выбирает только получателей с включённым доступом Viewer. Кнопка «Выгрузить всех врачей в один HTML» выбирает все доступные периоды, три типа страниц и каждого врача с личной «Выработкой», даже при выключенном обычном доступе; также включённого заведующего без личной выработки, если у его отделения есть отчёты. Main проверяет полноту списка личных страниц и только для этой полной HTML-публикации разрешает выдачу ключа неактивному врачу. Флаги доступа и действующие PIN в SQLite не меняются; администраторский PIN для автономного HTML вводится повторно, потому что открытым текстом не хранится. Совместимость сохранять.
...
- Мобильный экспорт включает до шести последних периодов с персональной «Выработкой». Новые публикации передают `coverage`, `preliminary`, `missing`, признак заданной цели и числовой контракт `numbers` v1 с единицами и методикой; старые публикации без этих полей читаются. Необязательные деревья, графики, история KPI и комментарии совместимы со старыми публикациями. Пропуски не заменять нулями. Серверная выдача передаёт каталог периодов и только запрошенный период, локальный файл сохраняет все периоды.
...
- `kbSummary` и прежние нормативы B–F по-прежнему управляют KPI, баллами, сводками и мобильными публикациями; прежние Viewer-пакеты самодостаточны и сохраняют старое представление. Эти группы могут пересекаться. Методика балла и мобильных групп обозначена `legacy-overlap-v1`, новое разбиение Admin/Viewer — `partition-v1-36m`; PWA поясняет различие. Прежняя таблица пяти групп убрана из настроек; сохранённые параметры B–F не изменяются при сохранении четырёх групп. Не заменять формулы KPI и мобильной публикации новым разбиением без отдельного решения о формулах и совместимости.
- Admin `adminDoctorDynamics` показывает январь–выбранный месяц; `computeDoctorDynamics` используется другими потребителями для шестимесячной динамики. История карточек — `doctorMetricDynamics`. Полностью отсутствующие календарные месяцы сохраняются как пропуски, сравнение с предыдущим месяцем не перескакивает через них; средние подписаны своим календарным окном.
...
Печать Admin/Viewer использует A4 альбомный: каждый вектор врача начинается с новой страницы, строки длинных таблиц переносятся между страницами, высота прокручиваемых областей и графиков ограничивается печатной страницей. Chromium PDF использует те же правила CSS.
